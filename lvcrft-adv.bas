10 REM LOVECRAFT TEXT ADVENTURE LEIKUR
20 REM *******************************
70 V=25:W=36:G=18
80 GOSUB 1600
90 CLS:PRINT "THE SHADOW OVER INNSMOUTH"
100 PRINT "--------------------------"
...
170 PRINT
180 FOR I=1 TO G
190 IF L(I)=RM AND F(I)=0 THEN PRINT "YOU CAN SEE ";0$(I);" HERE"
200 NEXT I
210 PRINT "======================="
220 PRINT M$:M$="WHAT"
...
270 IF MID$(Q$,I+1,1)<>" " AND V$<>"" THEN W$=MID$(Q$,I+l,LEN(Q$)-1):I=LEN(Q$)
280 NEXT I
290 IF W$="" THEN V$=Q$
300 FOR I=1 TO V
...
800 IF RM=45 AND C(1)=1 AND F(34) THEN M$="A MAGICAL BARRIER TO THE WEST":RETURN
...
840 IF (RM>26 ABD RM<30) AND F(0)=0 THEN M$="TOO DARK TO MOVE":RETURN
...
910 IF (U$="E" AND D=4 AND F(35)=0) THEN RM=RM+8:F(35)=1
920 NEXT I
930 M$="OK"
...
1050 IF RM=38 AND OB=32 THEN M$="THAT'S CREEPY YO!":F(2)=0
1060 RETURN
...
1150 IF (OB=3 OR OB=36) AND C(3)=1 AND F(34)=0 THEN M$="USE THIS WORD WITH CARE: BUTT"
1160 IF C(5)=1 AND OB=5 THEN M$="THE SCRIPT IS IN ALIEN TONGUE"
1170 RETURN

1180 M$="OK '"+W$+"'"
1190 IF C(3)=1 AND OB=34 THEN M$="*MAGIC OCCURS*":IF RM<>45 THEN RM=RND(63)
1200 IF C(3)=1 AND OB=34 AND RM=45 THEN F(34)=1
1210 RETURN

1220 IF C(12)=1 THEN M$="YOU MADE A HOLE"
1230 IF C(12)=1 AND RM=30 THEN M$="DUG THE BARS OUT":D$="HOLE IN WALL":R$(RM)="NSE"
1240 RETURN

1250 IF C(14) <>1 AND RM=7 THEN M$="THIS IS NO TIME TO PLAY MAGES!"
1260 IF OB=14 AND C(14)=1 THEN M$="YOU SWUNG IT"
1270 IF OB=13 AND C(13)=1 THEN M$="WHOOOSH!"
...
1440 IF F(27)=1 AND F(24)=1 THEN M$="WHIIIZ - SUCKED UP DEM GHOSTS!":F(27)=0
1450 RETURN
...
1610 DIMC(W),L(G),F(W)
1620 DATA46,38,35,50,13,18,28,42,10,25,26,4,2,7,47,60,43,32
1630 FOR I=1 TO G
1640 READ L(I)
1650 NEXT I
1660 DATAHELP,CARRYING?,GO,N,S,W,E,U,D,GET,TAKE,OPEN,EXAMINE,READ,SAY
1665 DATADIG,SWING,CLIMB,LIGHT,UNLIGHT,SPRAY,USE,UNLOCK,LEAVE,SCORE
1680 FOR I=1 TO V
1690 READ V$(I)
1700 NEXT I
